...
export default class StockProductosService {
  ...
  async createStockProductos(formData: any) {
    ...
    const insertData = `INSERT INTO ArticuloManufacturado(tiempoEstimadoCocina, denominacion, precioVenta, imagen, descripcion, idRubro, receta)
  ...
  async getIngredientesProductos(idArticuloManufacturado: string) {
    const query = `SELECT C.idCantIngredint, C.cantidad, A.unidadMedida, A.denominacion, R.denominacion AS RubroIngre 
  ...
  async getCantidadDisponibleProducto(idArticuloManufacturado: string) {
    const queryCantidad = `SELECT C.cantidad, A.stockActual FROM CantidadIngredientes c 
    INNER JOIN ArticuloInsumo A ON C.idArticuloInsumo = A.idArticuloInsumo
    WHERE idArticuloManufac = ${idArticuloManufacturado}`;

    let cantidadDisponible: number = 30;
    const listaCantidadIngredientes = await this.conectionLegacy.query(
      queryCantidad
    );
    //return listaCantidadIngredientes;
  ...
  async getIngredientes(rubro: string) {
    const query = `SELECT a.idArticuloInsumo, a.denominacion, a.unidadMedida
  ...
  async createIngredientesProductos(formData: any) {
    const insertData = `INSERT INTO CantidadIngredientes(cantidad, idArticuloInsumo, idArticuloManufac)
  ...
  async saveImg(userData: any) {
    const directory =
      "C:/Users/lucerom/Documents/ProyectoFinal/GitHub/BuenSaborFront/public";
